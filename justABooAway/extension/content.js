...
const GHOST_HAUNT_URL = chrome.runtime.getURL("icons/HauntedHead.gif"); // <-- Updated haunt asset
...
let isDying         = false;
let isHaunting      = false; // <-- Added haunt state
...
let ghostHauntText = null; // <-- Added haunt text element
...
function connectSocket(pairCode) {
    ...
    socket.addEventListener("message", (event) => {
        ...
        if (data.type === "pet-action") {
            ...
            // ========================================================
            //  ▼▼▼ UPDATED HAUNT ACTION BLOCK ▼▼▼
            // ========================================================
            } else if (data.action === "haunt-action") {
                console.log("[JustABooAway] BOO! We've been haunted (content)!");
                
                if (isDying) return; // Don't haunt a dying ghost
                if (!ghost) addGhost(false); // Show ghost if hidden
                if (!ghost) return; // Stop if ghost couldn't be added
                ...
                ensureHauntTextOverlay();

                // Get message from payload, or use a default
                const hauntMessage = (data.payload && data.payload.message) 
                                     ? data.payload.message 
                                     : "BOO!";
                
                // Apply effects
                ghost.src = GHOST_HAUNT_URL; // Set the haunt GIF
                if (ghostHauntText) {
                    ghostHauntText.textContent = hauntMessage; // Set the text
                    ghostHauntText.style.display = "block";
                }
                
                const HAUNT_DURATION_MS = 5000; // 5 seconds
                
                // Set timer to end the haunt
                setTimeout(() => {
                    isHaunting = false;
                    if (ghostHauntText) {
                        ghostHauntText.style.display = "none";
                    }
                    // Only revert if ghost still exists and isn't dying
                    if (ghost && !isDying) {
                ...
                // You can still send this if your background script needs it
                chrome.runtime.sendMessage({ action: "receiveHaunt" });
            }
            // ========================================================
            //  ▲▲▲ END UPDATED HAUNT ACTION BLOCK ▲▲▲
            // ========================================================
...
function startIdleAnimation() {
    ...
    function frame(timestamp) {
        // Updated to check for isHaunting
        if (!ghost || !ghostWrapper || isDying || isHaunting) {
        ...
        // Updated to check for isHaunting
        if (effect === null && !isDying && !isHaunting && ghost.src !== GHOST_IDLE_URL) {
            ghost.src = GHOST_IDLE_URL;
        }
...
function triggerEffect(kind) {
    // Updated to check for isHaunting
    if (!ghost || isDying || isHaunting) return;
...
// ========================================================
//  ▼▼▼ NEW HAUNT TEXT OVERLAY FUNCTION ▼▼▼
// ========================================================
function ensureHauntTextOverlay() {
    if (!ghostWrapper) return;
    if (ghostHauntText) return;

    ghostHauntText = document.createElement("div");
    ghostHauntText.id = "ghost-overlay-haunt-text";
    ghostHauntText.style.position = "absolute";
    ghostHauntText.style.width    = "200px"; // Make it wide enough
    ghostHauntText.style.left     = "50%";
    ghostHauntText.style.top      = "-20px"; // Position it above the ghost
    ghostHauntText.style.transform = "translate(-50%, -100%)"; // Center it
    ghostHauntText.style.pointerEvents = "none";
    ghostHauntText.style.display  = "none";
    ghostHauntText.style.color    = "white";
    ghostHauntText.style.fontFamily = "Arial, sans-serif";
    ghostHauntText.style.fontSize = "16px";
    ghostHauntText.style.fontWeight = "bold";
    ghostHauntText.style.textAlign = "center";
    ghostHauntText.style.textShadow = "2px 2px 4px rgba(0,0,0,0.8)"; // Make it readable

    ghostWrapper.appendChild(ghostHauntText);
}
// ========================================================
//  ▲▲▲ END NEW HAUNT TEXT OVERLAY FUNCTION ▲▲▲
// ========================================================
...
function addGhost(broadcast = true) {
    ...
    isHaunting = false; // <-- Reset haunt state on add
...
function removeGhost(broadcast = true) {
    ...
    isHaunting = false; // <-- Cancel haunt on remove
    isDying = true;
    stopIdleAnimation();
    effect = null;
    ...
    setTimeout(() => {
        ...
        ghostGlasses = null;
        ghostSpider  = null;
        ghostHauntText = null; // <-- Clear haunt text element
        isDying      = false;
        cancelSpider();
...
chrome.runtime.onMessage.addListener((msg) => {
    ...
    if (msg.action === "toggleGhost") {
        ...
        const hauntMessage = "never gonna give you up";
        
        // Send the action with the message as a payload
        sendPetAction("haunt-action", { message: hauntMessage });
